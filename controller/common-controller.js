                ...
                if (response) {
                    ...
                    if (isActiveUser) {
                        ...
                        if (sort && limit && offset) {
                            ...
                            if(semester!=undefined && typeof semester === 'string'){
                                    const result=await executer(`select * from class_master where semester=${semester} order by semester ${sort} limit ${limit} offset ${offset}`)

                                    res.status(200).json({ status: "success", message: 'class fetched successfully.', data: result })
                            }else{
                                //all sem
                                const result=await executer(`select * from class_master order by semester ${sort} limit ${limit} offset ${offset}`)
